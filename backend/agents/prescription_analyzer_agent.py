...
class PrescriptionAnalyzerAgent(BaseAgent):
    ...
    def _extract_medications(self, text: str) -> List[Dict[str, Any]]:
        ...
        # If no medications found, generate dummy data for testing
        if not medications:
            logger.info("No medications extracted, generating dummy data for testing")
            medications = self._generate_dummy_medications()
    ...
    def _generate_dummy_medications(self) -> List[Dict[str, Any]]:
    ...
    async def analyze_prescription(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            ...
            # Optionally refine extracted text using Gemini via RAGService for better parsing
            try:
                if hasattr(self.rag_service, 'refine_text_with_gemini') and extracted_text:
                    refined = await self.rag_service.refine_text_with_gemini(extracted_text)
                    if refined and isinstance(refined, str) and refined.strip():
                        extracted_text = refined
                        logger.debug("Extracted text refined via Gemini")
            except Exception as e:
                logger.debug(f"Gemini refinement skipped or failed: {e}")
            ...
            dummy_data_used = any(med.get("is_dummy", False) for med in prescription_data.medications)
