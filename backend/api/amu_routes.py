...
class AMUTrackingAgent(BaseAgent):
    ...
    async def analyze_animal_usage(self, animal_id: str, prescriptions: List[Dict], treatments: List[Dict]) -> Dict[str, Any]:
        """Analyze antimicrobial usage for a specific animal based on prescriptions and treatments"""
        try:
            usage_summary = {
                "animal_id": animal_id,
            ...
            # Extract antimicrobials from prescriptions
            antimicrobials = []
            for prescription in prescriptions:
                medicines = prescription.get("medicines", [])
                for medicine in medicines:
                    med_name = medicine.get("name", "").lower()
                    # Check if medicine is an antimicrobial
                    if any(drug in med_name for drug in self.guidelines["max_daily_dosage"].keys()):
                        antimicrobials.append({
                            ...
                            "type": medicine.get("medicineType", "antibiotic")
                        })
            
            # Extract antimicrobials from treatments
            ...
            usage_summary["antimicrobials_used"] = antimicrobials
            usage_summary["total_antimicrobials"] = len(antimicrobials)
            
            # Assess risk level based on number and types of antimicrobials
            if len(antimicrobials) == 0:
                usage_summary["risk_level"] = "none"
                usage_summary["recommendations"].append("No antimicrobial usage detected")
            elif len(antimicrobials) <= 2:
                usage_summary["risk_level"] = "low"
                usage_summary["recommendations"].extend([
                    "Monitor withdrawal periods for current medications",
                    "Maintain accurate treatment records"
                ])
            elif len(antimicrobials) <= 5:
                usage_summary["risk_level"] = "medium"
                usage_summary["recommendations"].extend([
                    "Review antimicrobial usage patterns",
                    "Consider antimicrobial stewardship protocols",
                    "Monitor for resistance development"
                ])
            else:
                usage_summary["risk_level"] = "high"
                usage_summary["recommendations"].extend([
                    "Immediate review of antimicrobial usage required",
                    "Consult with veterinary antimicrobial stewardship program",
                    "Implement resistance monitoring protocols"
                ])
            
            # Check for critical drugs
            critical_used = [am for am in antimicrobials if any(critical in am.get("name", "").lower() for critical in self.critical_drugs)]
            if critical_used:
                usage_summary["critical_drugs_used"] = len(critical_used)
                usage_summary["risk_level"] = "high"
                usage_summary["recommendations"].append("Critical antimicrobials detected - require special monitoring")
            
            # Generate compliance assessment
            if len(antimicrobials) > 5 or critical_used:
                usage_summary["compliance_status"] = "requires_review"
            
            return usage_summary
            
        except Exception as e:
            logger.error(f"Error analyzing animal usage: {e}")
            return {"error": f"Failed to analyze usage: {str(e)}"}
    ...
    async def execute(self, task: str, context: dict) -> dict:
        ...
        try:
            ...
            elif task == "analyze_usage":
                # New task for animal-centric API
                return await self.analyze_animal_usage(
                    context.get("animal_id", ""),
...
# --- API router wrapper for AMU agent (keeps file backward-compatible) ---
from fastapi import APIRouter, HTTPException
from typing import Dict, Any

router = APIRouter(prefix="/api/v1/amu", tags=["amu"])

amu_agent = None

def set_amu_agent(agent):
    """Set the module-level AMU agent instance used by the API endpoints"""
    global amu_agent
    amu_agent = agent


@router.get("/status")
async def get_amu_status() -> Dict[str, Any]:
    if not amu_agent:
        return {"status": "not_initialized", "message": "AMU agent not set"}
    try:
        info = await amu_agent.get_agent_info()
        return {"status": "success", "agent": info}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/analyze")
async def analyze_prescription_usage(prescription_data: Dict[str, Any]):
    if not amu_agent:
        raise HTTPException(status_code=503, detail="AMU agent not initialized")
    try:
