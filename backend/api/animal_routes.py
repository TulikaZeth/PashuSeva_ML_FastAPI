...
async def get_animal_amu_analysis(tag_no: str):
    ...
    try:
        ...
        # Extract prescription/treatment data for AMU analysis
        prescriptions = animal_data.get("prescriptions", [])
        treatments = animal_data.get("treatments", [])
        
        if not amu_agent:
            # Provide manual analysis if agent not available
            antimicrobials_used = []
            ...
            return {
                ...
                "amuAnalysis": {
                    ...
                    "riskLevel": "low" if len(antimicrobials_used) <= 3 else "medium" if len(antimicrobials_used) <= 6 else "high",
                    "recommendations": [
        ...
        # Use AMU agent if available
        context = {
            "animal_id": tag_no,
            "prescriptions": prescriptions,
            "treatments": treatments
        }
        result = await amu_agent.execute("analyze_usage", context)
        return {
            "tagNo": tag_no,
            "amuAnalysis": result,
            "timestamp": datetime.now().isoformat()
